...
function R(e, t) {
    let i = e.target;
    if (i instanceof Text && (i = i.parentElement), i instanceof Element && e.currentTarget instanceof Element) {
        const s = i.closest(t);
...
function D(e, t, i, s = {}) {
    ...
    const {
        once: r,
        ...a
    } = s, l = o instanceof Document ? o.documentElement : o, c = !!(typeof s == "object" ? s.capture : s), h = m => {
    ...
    x(!0, l, i, u) || l.addEventListener(t, h, a), n ? .addEventListener("abort", () => {
        x(!1, l, i, u)
    })
}
...
const I = (e, t) => String(e).toLowerCase().replace(/[\s/_.]+/g, "-").replace(/[^\w-]+/g, "").replace(/--+/g, "-").replace(/^-+|-+$/g, "") || t || "",
    S = ({
        hash: e
    ...
    b = (e = null, t = {}) => {
        ...
        const i = p({}, window.history.state || {}, {
            url: e,
            random: Math.random(),
            source: "swup"
        }, t);
...
class E extends Error {
    constructor(t, i) {
        super(t), this.url = void 0, this.status = void 0, this.aborted = void 0, this.timedOut = void 0, this.name = "FetchError", this.url = i.url, this.status = i.status, this.aborted = i.aborted || !1, this.timedOut = i.timedOut || !1
    }
}
async function B(e, t = {}) {
    ...
    const {
        visit: s = this.visit
    } = t, n = p({}, this.options.requestHeaders, t.headers), o = (i = t.timeout) != null ? i : this.options.timeout, r = new AbortController, {
        signal: a
    } = r;
    t = p({}, t, {
        headers: n,
    ...
    try {
        l = await this.hooks.call("fetch:request", s, {
            url: e,
            options: t
        }, (w, {
            url: k,
            options: y
        }) => fetch(k, y)), h && clearTimeout(h)
    } catch (w) {
        throw c ? (this.hooks.call("fetch:timeout", s, {
            url: e
        }), new E(`Request timed out: ${e}`, {
            url: e,
            timedOut: c
        })) : w ? .name === "AbortError" || a.aborted ? new E(`Request aborted: ${e}`, {
            url: e,
            aborted: !0
        }) : w
    }
    ...
    if (u === 500) throw this.hooks.call("fetch:error", s, {
        status: u,
        response: l,
        url: d
    }), new E(`Server error: ${d}`, {
        status: u,
        url: d
    });
    if (!m) throw new E(`Empty response: ${d}`, {
        status: u,
        url: d
    });
    const {
    ...
    return !s.cache.write || t.method && t.method !== "GET" || e !== g || this.cache.set(f.url, f), f
}
class _ {
    constructor(t) {
    ...
    set(t, i) {
        ...
        }), this.pages.set(t, i), this.swup.hooks.callSync("cache:set", void 0, {
            page: i
        })
    }
    update(t, i) {
        t = this.resolve(t);
        const s = p({}, this.get(t), i, {
            url: t
        });
        this.pages.set(t, s)
    }
...
class G {
    ...
    parseCallArgs(t, i, s, n) {
        return i instanceof M || typeof i != "object" && typeof s != "function" ? [i, s, n] : [void 0, i, s]
    }
    ...
    runSync(t, i = this.swup.visit, s, n = !1) {
        ...
        for (const {
            ...
            if (i == null || !i.done) {
                ...
                try {
                    const h = a(i, s, l);
                    o.push(h), N(h) && console.warn(`Swup will not await Promises in handler for synchronous hook '${r}'.`)
                } catch (h) {
                    if (n) throw h;
                    console.error(`Error in hook '${r}':`, h)
                }
    ...
    sortRegistrations(t, i) {
        var s, n;
        return ((s = t.priority) != null ? s : 0) - ((n = i.priority) != null ? n : 0) || t.id - i.id || 0
    }
...
const J = e => {
        ...
        let i = document.getElementById(e) || document.getElementById(t) || L(`a[name='${CSS.escape(e)}']`) || L(`a[name='${CSS.escape(t)}']`);
...
async function X({
    ...
    let i = [];
    if (t) i = Array.from(t);
    else if (e && (i = H(e, document.body), !i.length)) return void console.warn(`[swup] No elements found matching animationSelector \`${e}\``);
    const s = i.map(n => function(o) {
        const {
            type: r,
            timeout: a,
            propCount: l
        ...
        return !(!r || !a) && new Promise(c => {
            ...
            const m = () => {
                ...
                g = f => {
                    f.target === o && ((performance.now() - u) / 1e3 < f.elapsedTime || ++d >= l && m())
                };
            setTimeout(() => {
                d < l && m()
            }, a + 1), o.addEventListener(h, g)
        })
    }(n));
...
function Q(e, t = {}, i = {}) {
    ...
    const {
        url: s,
        hash: n
    } = v.fromUrl(e), o = this.createVisit(p({}, i, {
        to: s,
        hash: n
    }));
...
async function Y(e, t = {}) {
    ...
    var o, r;
    typeof n == "string" && (e.animation.name = n), e.meta = t.meta || {}, typeof t.cache == "object" ? (e.cache.read = (o = t.cache.read) != null ? o : e.cache.read, e.cache.write = (r = t.cache.write) != null ? r : e.cache.write) : t.cache !== void 0 && (e.cache = {
        read: !!t.cache,
        write: !!t.cache
    }), delete t.cache;
    try {
        await this.hooks.call("visit:start", e, void 0), e.state = 3;
        const a = this.hooks.call("page:load", e, {
            options: t
        }, async (c, h) => {
            let u;
            return c.cache.read && (u = this.cache.get(c.to.url)), h.page = u || await this.fetchPage(c.to.url, h.options), h.cache = !!u, h.page
        });
        a.then(({
            html: c
        }) => {
            e.advance(5), e.to.html = c, e.to.document = new DOMParser().parseFromString(c, "text/html")
        });
        const l = e.to.url + e.to.hash;
        if (e.history.popstate || (e.history.action === "replace" || e.to.url === this.location.url ? b(l) : (this.currentHistoryIndex++, j(l, {
                index: this.currentHistoryIndex
            }))), this.location = v.fromUrl(l), e.history.popstate && this.classes.add("is-popstate"), e.animation.name && this.classes.add(`to-${I(e.animation.name)}`), e.animation.wait && await a, e.done || (await this.hooks.call("visit:transition", e, void 0, async () => {
...
const Z = async function(e) {
        ...
        }), await this.hooks.call("animation:out:await", e, {
            skip: !1
        }, (t, {
            skip: i
        }) => {
    ...
    tt = function(e) {
        ...
        const n = H('[data-swup-persist]:not([data-swup-persist=""])'),
            o = e.containers.map(r => {
                const a = document.querySelector(r),
                    l = i.querySelector(r);
                return a && l ? (a.replaceWith(l.cloneNode(!0)), !0) : (a || console.warn(`[swup] Container missing in current document: ${r}`), l || console.warn(`[swup] Container missing in incoming document: ${r}`), !1)
            }).filter(Boolean);
        return n.forEach(r => {
    ...
    et = function(e) {
        const t = {
            ...
            n = i ? ? e.to.hash;
        let o = !1;
        return n && (o = this.hooks.callSync("scroll:anchor", e, {
            hash: n,
            options: t
        }, (r, {
            hash: a,
        ...
        })), s && !o && (o = this.hooks.callSync("scroll:top", e, {
            options: t
        }, (r, {
            options: a
        }) => (window.scrollTo(p({
    ...
    it = async function(e) {
        if (e.done) return;
        const t = this.hooks.call("animation:in:await", e, {
            skip: !1
        }, (i, {
            skip: s
        }) => {
            if (!s) return this.awaitAnimations({
                selector: i.animation.selector
            })
        });
        await q(), await this.hooks.call("animation:in:start", e, void 0, () => {
    ...
    st = async function(e, t) {
        ...
        this.isSameResolvedUrl(S(), i) || (b(i), this.location = v.fromUrl(i), e.to.url = this.location.url, e.to.hash = this.location.hash), await this.hooks.call("content:replace", e, {
            page: t
        }, (s, {}) => {
            ...
            s.animation.animate && (this.classes.add("is-changing", "is-animating", "is-rendering"), s.animation.name && this.classes.add(`to-${I(s.animation.name)}`))
...
function ot(e) {
    ...
    if (t) return t.unmount(), t._afterUnmount && t._afterUnmount(), this.plugins = this.plugins.filter(i => i !== t), this.plugins;
...
class ct {
    ...
    constructor(t = {}) {
        var i, s;
        this.version = "4.8.2", this.options = void 0, this.defaults = ht, this.plugins = [], this.visit = void 0, this.cache = void 0, this.hooks = void 0, this.classes = void 0, this.location = v.fromUrl(window.location.href), this.currentHistoryIndex = void 0, this.clickDelegate = void 0, this.navigating = !1, this.onVisitEnd = void 0, this.use = nt, this.unuse = ot, this.findPlugin = rt, this.log = () => {}, this.navigate = Q, this.performNavigation = Y, this.createVisit = K, this.delegateEvent = W, this.fetchPage = B, this.awaitAnimations = X, this.renderPage = st, this.replaceContent = tt, this.animatePageIn = it, this.animatePageOut = Z, this.scrollToContent = et, this.getAnchorElement = J, this.getCurrentUrl = S, this.resolveUrl = at, this.isSameResolvedUrl = lt, this.options = p({}, this.defaults, t), this.handleLinkClick = this.handleLinkClick.bind(this), this.handlePopState = this.handlePopState.bind(this), this.cache = new _(this), this.classes = new z(this), this.hooks = new G(this), this.visit = this.createVisit({
            to: ""
        }), this.currentHistoryIndex = (i = (s = window.history.state) == null ? void 0 : s.index) != null ? i : 1, this.enable()
    }
    async enable() {
        var t;
        const {
        ...
        for (const [s, n] of Object.entries(this.options.hooks)) {
            ...
            this.hooks.on(o, n, r)
        }((t = window.history.state) == null ? void 0 : t.source) !== "swup" && b(null, {
            index: this.currentHistoryIndex
        }), await q(), await this.hooks.call("enable", void 0, void 0, () => {
    ...
    shouldIgnoreVisit(t, {
        ...
        return n !== window.location.origin || !(!i || !this.triggerWillOpenNewWindow(i)) || !!this.options.ignoreVisit(o + r, {
            el: i,
            event: s
        })
    }
    handleLinkClick(t) {
        ...
        const r = this.createVisit({
            to: n,
            hash: o,
            el: i,
            event: t
        });
        t.metaKey || t.ctrlKey || t.shiftKey || t.altKey ? this.hooks.callSync("link:newtab", r, {
            href: s
        }) : t.button === 0 && this.hooks.callSync("link:click", r, {
            el: i,
            event: t
        }, () => {
            var a;
            const l = (a = r.from.url) != null ? a : "";
            t.preventDefault(), n && n !== l ? this.isSameResolvedUrl(n, l) || this.performNavigation(r) : o ? this.hooks.callSync("link:anchor", r, {
                hash: o
            }, () => {
                b(n + o), this.scrollToContent(r)
            }) : this.hooks.callSync("link:self", r, void 0, () => {
                this.options.linkToSelf === "navigate" ? this.performNavigation(r) : (b(n), this.scrollToContent(r))
            })
        })
    }
    handlePopState(t) {
        ...
        if (this.options.skipPopStateHandling(t) || this.isSameResolvedUrl(S(), this.location.url)) return;
        const {
            url: a,
            hash: l
        } = v.fromUrl(r), c = this.createVisit({
            to: a,
            hash: l,
            event: t
        });
        ...
        h && h !== this.currentHistoryIndex && (c.history.direction = h - this.currentHistoryIndex > 0 ? "forwards" : "backwards", this.currentHistoryIndex = h), c.animation.animate = !1, c.scroll.reset = !1, c.scroll.target = !1, this.options.animateHistoryBrowsing && (c.animation.animate = !0, c.scroll.reset = !0), this.hooks.callSync("history:popstate", c, {
            event: t
        }, () => {
