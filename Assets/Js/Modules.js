...
function Observer(_target, _margin, _threshold, _inCB, _outCB) {
    ...
    if (GLB._hasIntersectionObs) {
        ...
        _observer = new IntersectionObserver(intersected, {
            root: null,
            rootMargin: _m,
...
function HomeTime(_me) {
    ...
    var _type = "chars",
        _splittext;
    if (!GLB._isBot) {
        _splittext = new SplitText(_title, {
            type: _type
        });
    ...
    if (GLB._hasTouch) {
        ...
        _knob.addEventListener("touchmove", mmove, {
            passive: false
        });
    }
    ...
    _this.destroy = function() {
        ...
        if (GLB._hasTouch) {
            ...
            _knob.removeEventListener("touchmove", mmove, {
                passive: false
            });
        }
        if (!GLB._isBot) {
...
function TitleAnim(_me) {
    ...
    if (_title && !GLB._isBot) {
        ...
        _chars = _splittext.chars, _numChars = _chars.length;
        for (var i = 0; i < _numChars; i++) gsap.set(_chars[i], {
            force3D: true,
            x: 32,
            opacity: 0
        });
    ...
    function inView() {
        if (!_title) return;
        for (var i = 0; i < _numChars; i++) gsap.to(_chars[i], .6, {
            force3D: true,
            x: 0,
            opacity: 1,
            ease: "cubic",
...
function Parallax(_me) {
    var _this = this;
    var _inView = false,
        _layoutTimer, _scrolledLocal, _offset, _offPos;
    ...
    function betweenBreakpoint(e) {
        if (GLB._isMobile) gsap.set(_target, {
            x: 0,
            force3D: true,
...
function ProjectForm(_me) {
    ...
    var _closeTimer, _thankyouOn = false,
        _dropzone;
